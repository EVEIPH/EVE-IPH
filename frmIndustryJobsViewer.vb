...
ExitSub:
    ...
    Private Sub RefreshCharacterList(ByVal UpdateAPIData As Boolean)
        ...
        SQL = SQL & "LEFT JOIN (SELECT SUM({0}) + 1 AS RESEARCH_LINES, CHARACTER_ID AS CHAR_ID FROM CHARACTER_SKILLS WHERE SKILL_TYPE_ID IN (3406,24624) GROUP BY CHARACTER_ID) AS I ON I.CHAR_ID = ECD.CHARACTER_ID "
        ...
        SQL = SQL & "LEFT JOIN (SELECT SUM({0}) + 1 AS INDUSTRY_LINES, CHARACTER_ID AS CHAR_ID FROM CHARACTER_SKILLS WHERE SKILL_TYPE_ID IN (3387,24625) GROUP BY CHARACTER_ID) AS K ON K.CHAR_ID = ECD.CHARACTER_ID "
        ...
        SQL = SQL & "LEFT JOIN (SELECT SUM({0}) + 1 AS REACTION_LINES, CHARACTER_ID AS CHAR_ID FROM CHARACTER_SKILLS WHERE SKILL_TYPE_ID IN (45748,45749) GROUP BY CHARACTER_ID) AS M ON M.CHAR_ID = ECD.CHARACTER_ID "
